...
public class TestCorridaRepositoryCompleto {
    ...
    public static void main(String[] args) {
        ...
        corridaRepo.findAll().forEach(c -> System.out.println(c.getId() + ": " + c.getOrigem() + " -> " + c.getDestino()));

        // ===== Testar findByMotoristaId =====
        System.out.println("\n=== Corridas do motorista 10 ===");
        corridaRepo.findByMotoristaId(10).forEach(c -> System.out.println(c.getId() + ": " + c.getOrigem() + " -> " + c.getDestino()));

        // ===== Testar findByPassageiroId =====
        System.out.println("\n=== Corridas do passageiro 1 ===");
        corridaRepo.findByPassageiroId(1).forEach(c -> System.out.println(c.getId() + ": " + c.getOrigem() + " -> " + c.getDestino()));

        // ===== Testar findByStatus =====
        System.out.println("\n=== Corridas pendentes ===");
        corridaRepo.findByStatus(CorridaStatus.PENDENTE).forEach(c -> System.out.println(c.getId() + ": " + c.getOrigem() + " -> " + c.getDestino()));
        ...
        corridaRepo.findEmAndamento().forEach(c -> System.out.println(c.getId() + ": " + c.getOrigem() + " -> " + c.getDestino()));

        // ===== Testar findByOrigem =====
        System.out.println("\n=== Corridas com origem A ===");
        corridaRepo.findByOrigem("A").forEach(c -> System.out.println(c.getId() + ": " + c.getOrigem() + " -> " + c.getDestino()));

        // ===== Testar findByDestino =====
        System.out.println("\n=== Corridas com destino C ===");
        corridaRepo.findByDestino("C").forEach(c -> System.out.println(c.getId() + ": " + c.getOrigem() + " -> " + c.getDestino()));

        // ===== Testar findByOrigemAndDestino =====
        System.out.println("\n=== Corridas de A para C ===");
        corridaRepo.findByOrigemAndDestino("A", "C").forEach(c -> System.out.println(c.getId() + ": " + c.getOrigem() + " -> " + c.getDestino()));

        // ===== Testar findByCategoria =====
        System.out.println("\n=== Corridas categoria LUXO ===");
        corridaRepo.findByCategoria(Categoria.LUXO).forEach(c -> System.out.println(c.getId() + ": " + c.getOrigem() + " -> " + c.getDestino()));
